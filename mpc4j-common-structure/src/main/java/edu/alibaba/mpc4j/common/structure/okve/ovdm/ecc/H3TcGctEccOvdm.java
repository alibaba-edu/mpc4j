package edu.alibaba.mpc4j.common.structure.okve.ovdm.ecc;
 ...
 * @date 2021/09/10
 */
public class H3TcGctEccOvdm<T> extends AbstractEccOvdm<T> implements SparseEccOvdm<T> {
    /**
     * 3 sparse hashes
     */
    ...
    public ECPoint decode(ECPoint[] storage, T key) {
        ...
        // 三个哈希结果一定不同，计算3次求和
        value = value.add(storage[sparsePositions[0]]).add(storage[sparsePositions[1]]).add(storage[sparsePositions[2]]);
