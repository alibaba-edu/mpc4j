...
/**
 * 2^l选1-HOT协议发送方输出。服务端输入Δ，得到q_1,...,q_k，客户端输入选择元组(m_1,...,m_k)，得到t_0,...,t_k。各个参数满足：
 * <p>
 ...
 * @date 2022/5/23
 */
public class LhotSenderOutput implements LotSenderOutput {
    /**
     * 输入比特长度
     */
     ...
     * @return 发送方输出。
     */
    public static LhotSenderOutput create(int inputBitLength, byte[] delta, byte[][] qsArray) {
        LhotSenderOutput senderOutput = new LhotSenderOutput();
     ...
     * @return 数量为0的发送方输出。
     */
    public static LhotSenderOutput createEmpty(int inputBitLength, byte[] delta) {
        LhotSenderOutput senderOutput = new LhotSenderOutput();
        assert inputBitLength > 0 : "InputBitLength must be greater than 0: " + inputBitLength;
        senderOutput.inputBitLength = inputBitLength;
        senderOutput.inputByteLength = CommonUtils.getByteLength(inputBitLength);
     ...
     * 私有构造函数。
     */
    private LhotSenderOutput() {
        // empty
    }

    @Override
    public LhotSenderOutput split(int length) {
        int num = getNum();
        ...
        return LhotSenderOutput.create(inputBitLength, delta, qsSubArray);
    }
    ...
    public void merge(LotSenderOutput that) {
        assert that instanceof LhotSenderOutput;
        LhotSenderOutput thatOutput = (LhotSenderOutput) that;
