...
/**
 * OOS17-2^l选1-HOT协议发送方。
 *
 * @author Weiran Liu
 * @date 2022/6/8
 */
public class Oos17LhotSender extends AbstractLhotSender {
    /**
     * COT协议接收方
     */
    ...
    private void init() throws MpcAbortException {
        ...
        DataPacketHeader randomOracleKeyHeader = new DataPacketHeader(
            taskId, getPtoDesc().getPtoId(), Oos17LhotPtoDesc.PtoStep.SENDER_SEND_RANDOM_ORACLE_KEY.ordinal(), extraInfo,
    ...
    public LhotSenderOutput send(int num) throws MpcAbortException {
        ...
        DataPacketHeader matrixHeader = new DataPacketHeader(
            taskId, ptoDesc.getPtoId(), Oos17LhotPtoDesc.PtoStep.RECEIVER_SEND_MATRIX.ordinal(), extraInfo,
        ...
        DataPacketHeader correlateCheckHeader = new DataPacketHeader(
            taskId, getPtoDesc().getPtoId(), Oos17LhotPtoDesc.PtoStep.RECEIVER_SEND_CHECK.ordinal(), extraInfo,
            otherParty().getPartyId(), ownParty().getPartyId()
        );
        List<byte[]> correlateCheckPayload = rpc.receive(correlateCheckHeader).getPayload();
        LhotSenderOutput senderOutput = handleCorrelateCheckPayload(correlateCheckPayload);
    ...
    private LhotSenderOutput handleCorrelateCheckPayload(List<byte[]> correlateCheckPayload) throws MpcAbortException {
