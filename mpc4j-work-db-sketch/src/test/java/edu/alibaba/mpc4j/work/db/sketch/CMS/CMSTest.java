...
import edu.alibaba.mpc4j.work.db.sketch.CMS.v2.v2CMSPtoDesc;
...
public class CMSTest extends AbstractThreePartyMemoryRpcPto {
    ...
    public static Collection<Object[]> configurations() {
        ...
        configurations.add(new Object[]{
            v2CMSPtoDesc.getInstance().getPtoName() + "(semi-honest)",
            new v2CMSConfig.Builder(false).build(), false, true
        });
        return configurations;
    }
    /**
     * config of CMS protocol
     */
    ...
    public CMSTest(String name, CMSConfig config, boolean baseUseMac, boolean isInputBinary) {
        super(name);
        this.config = config;
    ...
    public void testSmallSize() {
        testOpi(false, SMALL_LOG_SKETCH_SIZE, SMALL_ELEMENT_BIT_LEN, SMALL_PAYLOAD_BIT_LEN, SMALL_UPDATE_NUM, SMALL_QUERY_NUM, isInputBinary);
    }

    @Test
    public void testMiddleSize() {
        testOpi(false, MIDDLE_LOG_SKETCH_SIZE, MIDDLE_ELEMENT_BIT_LEN, MIDDLE_PAYLOAD_BIT_LEN, MIDDLE_UPDATE_NUM, MIDDLE_QUERY_NUM, isInputBinary);
    }
    ...
    private void testOpi(boolean parallel, int logSketchSize, int elementBitLen, int payloadBitLen, int smallUpdateNum, int smallQueryNum, boolean isInputBinary) {
        ...
        try {
            ...
            CMSPartyThread[] threads = Arrays.stream(parties)
                .map(p -> new CMSPartyThread(p, updateKeys, queryKeys, elementBitLen, logSketchSize, payloadBitLen, isInputBinary))
            ...
            // todo we current only ensure the sum is correct
            CMS plainCms;
    ...
    private void verifyQuery(BigInteger[] updateKeys, BigInteger[] queryKeys, long[] queryRes) {
        ...
        // currently we only verify the result is not smaller than true result
