...
import static edu.alibaba.mpc4j.s2pc.aby.operator.corr.zl.ZlCorrFactory.ZlCorrType;
 ...
 * @date 2023/10/12
 */
public class ZlCorrMain {
    private static final Logger LOGGER = LoggerFactory.getLogger(ZlCorrMain.class);
    /**
     * task name
     */
    public static final String TASK_NAME = "ZL_CORR_TASK";
    /**
    ...
    public ZlCorrMain(Properties properties) {
    ...
    private void runServer(Rpc serverRpc, Party clientParty) throws Exception {
        ...
        ZlCorrConfig config = ZlCorrConfigUtils.createZlCorrConfig(properties);
        LOGGER.info("{} create result file", serverRpc.ownParty().getPartyName());
        String filePath = zlCorrType.name()
    ...
    private void warmupServer(Rpc serverRpc, Party clientParty, ZlCorrConfig config, int taskId) throws Exception {
        ZlCorrParty sender = ZlCorrFactory.createSender(serverRpc, clientParty, config);
        ...
        sender.corr(SquareZlVector.create(zl, xi, false));
        sender.getRpc().synchronize();
        sender.getRpc().reset();
        sender.destroy();
    ...
    private void runServer(Rpc serverRpc, Party clientParty, ZlCorrConfig config, int taskId,
        ...
        LOGGER.info(
            "{}: num = {}, bit-length = {}, parallel = {}, silent = {}",
            serverRpc.ownParty().getPartyName(), num, elementBitLength, parallel, silent
        );
        ZlCorrParty sender = ZlCorrFactory.createSender(serverRpc, clientParty, config);
        sender.setTaskId(taskId);
    ...
    private void warmupClient(Rpc clientRpc, Party serverParty, ZlCorrConfig config, int taskId) throws Exception {
        ZlCorrParty receiver = ZlCorrFactory.createReceiver(clientRpc, serverParty, config);
        ...
        receiver.corr(SquareZlVector.create(zl, xi, false));
        receiver.getRpc().synchronize();
    ...
    private void runClient(Rpc clientRpc, Party serverParty, ZlCorrConfig config, int taskId,
                           int num, int elementBitLength, boolean parallel, PrintWriter printWriter)
        throws MpcAbortException {
        boolean silent = PropertiesUtils.readBoolean(properties, "silent");
        LOGGER.info(
            "{}: num = {}, bit-length = {}, parallel = {}, silent = {}",
            clientRpc.ownParty().getPartyName(), num, elementBitLength, parallel, silent
        );
        ZlCorrParty receiver = ZlCorrFactory.createReceiver(clientRpc, serverParty, config);
        receiver.setTaskId(taskId);
