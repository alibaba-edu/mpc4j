...
import edu.alibaba.mpc4j.s2pc.pir.index.single.xpir.Mbfk16SingleIndexPirPtoDesc.PtoStep;
 ...
 * @date 2022/8/24
 */
public class Mbfk16SingleIndexPirClient extends AbstractSingleIndexPirClient {
    ...
    public Mbfk16SingleIndexPirClient(Rpc clientRpc, Party serverParty, Mbfk16SingleIndexPirConfig config) {
        super(Mbfk16SingleIndexPirPtoDesc.getInstance(), clientRpc, serverParty, config);
    }
    ...
    public List<byte[]> generateQuery(int index) {
        ...
        return Mbfk16SingleIndexPirNativeUtils.generateQuery(
    ...
    public byte[] decodeResponse(List<byte[]> response, int index, int elementBitLength) throws MpcAbortException {
        ...
        intStream.forEach(partitionIndex -> {
            long[] coeffs = Mbfk16SingleIndexPirNativeUtils.decryptReply(
    ...
    private List<byte[]> generateKeyPair() {
        List<byte[]> keyPair = Mbfk16SingleIndexPirNativeUtils.keyGen(params.getEncryptionParams());
