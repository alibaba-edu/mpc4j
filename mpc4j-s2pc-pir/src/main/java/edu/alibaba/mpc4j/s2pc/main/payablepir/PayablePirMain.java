package edu.alibaba.mpc4j.s2pc.main.payablepir;
...
public class PayablePirMain {
    ...
    private void warmupServer(Rpc serverRpc, Party clientParty, PayablePirConfig config, int taskId)
        ...
        Map<ByteBuffer, ByteBuffer> keywordValueMap = IntStream.range(0, WARMUP_SERVER_ELEMENT_SIZE)
            .boxed()
            .collect(Collectors.toMap(
                i -> ByteBuffer.wrap(IntUtils.intToByteArray(i)),
                i -> ByteBuffer.wrap(elementArray[i]),
                (a, b) -> b,
    ...
    private void runServer(Rpc serverRpc, Party clientParty, PayablePirConfig config, int taskId,
        ...
        Map<ByteBuffer, ByteBuffer> keywordValueMap = IntStream.range(0, serverElementSize)
            .boxed()
            .collect(Collectors.toMap(
                i -> ByteBuffer.wrap(IntUtils.intToByteArray(i)),
                i -> ByteBuffer.wrap(serverElementArray[i]),
                (a, b) -> b,
