...
import edu.alibaba.mpc4j.s2pc.pir.cppir.index.spam.SpamCpIdxPirPtoDesc.PtoStep;
...
/**
 * SPAM client-specific preprocessing index PIR client.
 *
 * @author Weiran Liu
 * @date 2023/9/4
 */
public class SpamCpIdxPirClient extends AbstractCpIdxPirClient {
    /**
     * chunk size
     */
    ...
    public SpamCpIdxPirClient(Rpc clientRpc, Party serverParty, SpamCpIdxPirConfig config) {
        super(SpamCpIdxPirPtoDesc.getInstance(), clientRpc, serverParty, config);
    }
    ...
    public void init(int n, int l, int maxBatchNum) throws MpcAbortException {
        ...
        stopWatch.start();
        chunkSize = SpamCpIdxPirUtils.getChunkSize(n);
        chunkNum = SpamCpIdxPirUtils.getChunkNum(n);
        assert chunkSize * chunkNum >= n
            : "chunkSize * chunkNum must be greater than or equal to n (" + n + "): " + chunkSize * chunkNum;
        roundQueryNum = SpamCpIdxPirUtils.getRoundQueryNum(n);
        m1 = SpamCpIdxPirUtils.getM1(n);
        m2 = SpamCpIdxPirUtils.getM2(n);
        stopWatch.stop();
    ...
    private void preprocessing() throws MpcAbortException {
        ...
        for (int chunkId = 0; chunkId < chunkNum; chunkId++) {
            ...
            Stream<SpamPrimaryHint> primaryHintStream = parallel ? Arrays.stream(primaryHints).parallel() : Arrays.stream(primaryHints);
            ...
            Stream<SpamBackupHint> backupHintStream = parallel ? backupHints.stream().parallel() : backupHints.stream();
