 ...
 * @date 2023/9/28
 */
public class PsiCaMain {
    private static final Logger LOGGER = LoggerFactory.getLogger(PsiCaMain.class);
    /**
     * protocol type name
     */
    public static final String PTO_TYPE_NAME = "PSI_CA";
    /**
    ...
    public PsiCaMain(Properties properties) {
    ...
    public void runServer(Rpc serverRpc, Party clientParty) throws MpcAbortException, IOException {
        ...
        PsiCaConfig config = PsiCaConfigUtils.createPsiCaConfig(properties);
        // 生成输入文件
    ...
    private void warmupServer(Rpc serverRpc, Party clientParty, PsiCaConfig config, int taskId)
        ...
        PsiCaServer<ByteBuffer> psiServer = PsiCaFactory.createServer(serverRpc, clientParty, config);
        ...
        psiServer.psiCardinality(serverElementSet, WARMUP_SET_SIZE);
        psiServer.getRpc().synchronize();
        psiServer.getRpc().reset();
        LOGGER.info("(warmup) {} finish", psiServer.ownParty().getPartyName());
    }

    public void runServer(Rpc serverRpc, Party clientParty, PsiCaConfig config, int taskId, boolean parallel,
        ...
        serverStopWatch.start();
        psiServer.psiCardinality(serverElementSet, clientSetSize);
        serverStopWatch.stop();
    ...
    private void warmupClient(Rpc clientRpc, Party serverParty, PsiCaConfig config, int taskId)
        throws MpcAbortException, IOException {
        Set<ByteBuffer> clientElementSet = readClientElementSet(WARMUP_SET_SIZE, WARMUP_ELEMENT_BYTE_LENGTH);
        PsiCaClient<ByteBuffer> psiClient = PsiCaFactory.createClient(clientRpc, serverParty, config);
        ...
        psiClient.psiCardinality(clientElementSet, WARMUP_SET_SIZE);
        // 同步并等待5秒钟，保证对方执行完毕
        psiClient.getRpc().synchronize();
    ...
    public void runClient(Rpc clientRpc, Party serverParty, PsiCaConfig config, int taskId, boolean parallel,
        ...
        clientStopWatch.start();
        psiClient.psiCardinality(clientElementSet, serverSetSize);
        clientStopWatch.stop();
