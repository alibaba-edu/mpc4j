...
import edu.alibaba.mpc4j.crypto.swhe.seal.serialization.SealSerializable;
import edu.alibaba.mpc4j.crypto.swhe.seal.utils.Constants;
import edu.alibaba.mpc4j.crypto.swhe.seal.utils.GaloisTool;
import edu.alibaba.mpc4j.crypto.swhe.seal.utils.RingLwe;
...
public class KeyGenerator {
    ...
    private void generateSk(boolean isInitialized) {
        ...
        if (!isInitialized) {
            ...
            // Generate secret key
            RnsIterator secretKeyRns = new RnsIterator(secretKey.data().getData(), coeffCount, coeffModulusSize);
    ...
    private void createRelinKeys(@SuppressWarnings("SameParameterValue") int count, boolean saveSeed, RelinKeys destination) {
        ...
        PolyIterator secretKey = new PolyIterator(secretKeyArray, coeffCount * coeffModulusSize, coeffCount, coeffModulusSize);
    ...
    private void createGaloisKeys(int[] galoisElts, boolean saveSeed, GaloisKeys destination) {
        ...
        for (int galoisElt : galoisElts) {
            ...
            RnsIterator rotatedSecretKey = RnsIterator.createZeroRns(coeffCount, coeffModulusSize);
            RnsIterator secretKeyRns = new RnsIterator(secretKey.data().getData(), coeffCount, coeffModulusSize);
            galoisTool.applyGaloisNttRnsIter(secretKeyRns, coeffModulusSize, galoisElt, rotatedSecretKey);
    ...
    private void computeSecretKeyArray(ContextData contextData, int maxPower) {
        ...
        // new Rns and Poly
        RnsIterator secretKeyArrayRns = new RnsIterator(secretKeyArray, coeffCount, coeffModulusSize);
        PolyIterator secretKeyPower = new PolyIterator(newSecretKeyArray, 0, newSize, coeffCount, coeffModulusSize);
        ...
        for (int i = oldSize - 1; i < oldSize - 1 + newSize - oldSize; i++) {
            PolyArithmeticSmallMod.dyadicProductCoeffModRns(
                    secretKeyPower.rnsIterators[i],
                    secretKeyArrayRns,
                    coeffModulusSize,
                    coeffModulus,
                    secretKeyPower.rnsIterators[i + 1]);
        }
        // Acquire new array
    ...
    private void generateOneKeySwitchKey(RnsIterator newKey,
        ...
        for (int i = 0; i < decomposeModCount; i++) {
            ...
            PolyArithmeticSmallMod.multiplyPolyScalarCoeffMod(newKey.coeffIterators[i], coeffCount, factor, keyModulus[i], temp);
            // ([-(as + e)]_q_0, a), ..., ([-(as + e) + new_keys * q_k]_q_i, a), ...,([-(as + e)]_q_{k-1}, a)
            CoeffIterator destinationIter = new CoeffIterator(destinations[index][i].data().data(), i * coeffCount, coeffCount);
    ...
    private void generateKeySwitchKeys(PolyIterator newKeys, int numKeys, KswitchKeys destination, boolean saveSeed) {
        ...
        for (int i = 0; i < numKeys; i++) {
            generateOneKeySwitchKey(newKeys.rnsIterators[i], destination.data(), i, saveSeed);
        }
    }
