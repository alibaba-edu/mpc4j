...
import edu.alibaba.mpc4j.crypto.swhe.seal.iterator.StrideIterator;
import edu.alibaba.mpc4j.crypto.swhe.seal.modulus.Modulus;
import edu.alibaba.mpc4j.crypto.swhe.seal.ntt.NttTables;
import edu.alibaba.mpc4j.crypto.swhe.seal.ntt.NttTool;
import edu.alibaba.mpc4j.crypto.swhe.seal.rns.RnsTool;
import edu.alibaba.mpc4j.crypto.swhe.seal.rq.PolyArithmeticSmallMod;
import edu.alibaba.mpc4j.crypto.swhe.seal.rq.PolyCore;
import edu.alibaba.mpc4j.crypto.swhe.seal.utils.Constants;
import edu.alibaba.mpc4j.crypto.swhe.seal.utils.GaloisTool;
import edu.alibaba.mpc4j.crypto.swhe.seal.utils.ScalingVariant;
import edu.alibaba.mpc4j.crypto.swhe.seal.utils.ValCheck;
import edu.alibaba.mpc4j.crypto.swhe.seal.zq.*;
...
public class Evaluator {
    ...
    public void negateInplace(Ciphertext encrypted) {
        ...
        PolyArithmeticSmallMod.negatePolyCoeffModPoly(encryptedPoly, encryptedSize, coeffModulus, encryptedPoly);
    }
    ...
    public void subInplace(Ciphertext encrypted1, Ciphertext encrypted2) {
        ...
        if (encrypted1.correctionFactor() != encrypted2.correctionFactor()) {
            ...
            PolyArithmeticSmallMod.multiplyPolyScalarCoeffModPoly(
                    encrypted1Poly, encrypted1.size(),
            ...
            if (encrypted1Size < encrypted2Size) {
                encrypted1Poly = PolyIterator.fromCiphertext(encrypted1, minCount);
                encrypted2Poly = PolyIterator.fromCiphertext(encrypted2, minCount);
                PolyArithmeticSmallMod.negatePolyCoeffModPoly(
                        encrypted2Poly,
                        encrypted2Size - minCount,
                        coeffModulus,
                        encrypted1Poly
                );
            }
    ...
    private void bfvMultiply(Ciphertext encrypted1, Ciphertext encrypted2) {
        ...
        PolyIterator encrypted1Q = PolyIterator.createZeroPoly(encrypted1Size, coeffCount, baseQSize);
        // Allocate space for a base Bsk output of behz_extend_base_convert_to_ntt for encrypted1
        PolyIterator encrypted1Bsk = PolyIterator.createZeroPoly(encrypted1Size, coeffCount, baseBskSize);
        ...
        PolyIterator encrypted2Q = PolyIterator.createZeroPoly(encrypted2Size, coeffCount, baseQSize);
        PolyIterator encrypted2Bsk = PolyIterator.createZeroPoly(encrypted2Size, coeffCount, baseBskSize);
        ...
        PolyIterator tempDestinationQ = PolyIterator.createZeroPoly(destinationSize, coeffCount, baseQSize);
        PolyIterator tempDestinationBsk = PolyIterator.createZeroPoly(destinationSize, coeffCount, baseBskSize);
        ...
        for (int i = 0; i < destinationSize; i++) {
            ...
            RnsIterator tempBsk = RnsIterator.createZeroRns(coeffCount, baseBskSize);

            // Step (7): divide by q and floor, producing a result in base Bsk
            rnsTool.fastFloorRnsIter(tempQBsk, tempBsk);

            // Step (8): use Shenoy-Kumaresan method to convert the result to base q and write to encrypted1
            rnsTool.fastBConvSkRnsIter(tempBsk, encrypted1Poly.rnsIterators[i]);
        }
    }
    ...
    private void behzExtendBaseConvertToNtt(Ciphertext encrypt, int size,
        ...
        for (int i = 0; i < size; i++) {
            ...
            // Lazy reduction
            NttTool.nttNegacyclicHarveyLazyRns(encryptedQ.rnsIterators[i], baseQSize, baseQNttTables);

            // Allocate temporary space for a polynomial in base {B_sk, {m_tilde}}
            RnsIterator temp = RnsIterator.createZeroRns(n, baseBskMTildeSize);

            // 1) Convert from base q to base {B_sk, {m_tilde}}
            PolyIterator encryptedPoly = PolyIterator.fromCiphertext(encrypt);
            rnsTool.fastBConvMTildeRnsIter(encryptedPoly.rnsIterators[i], temp);

            // (2) Reduce q-overflows in with Montgomery reduction, switching base to B_sk
            rnsTool.smMrqRnsIter(temp, encryptedBsk.rnsIterators[i]);
            // Transform to NTT form in base B_sk
            // Lazy reduction
            NttTool.nttNegacyclicHarveyLazyRns(encryptedBsk.rnsIterators[i], baseBskSize, baseBskNttTables);
        }
    }
    ...
    private void behzCiphertextProduct(PolyIterator in1, int in1Index, PolyIterator in2, int in2Index, int n,
        ...
        RnsIterator shiftedOutIter = destination.rnsIterators[desIndex];
        for (int j = 0; j < steps; j++) {
            // Create a shifted reverse iterator for the second input
            // 在这里每一次精确获取 in2 对应的 RnsIter
            RnsIterator shiftedReversedIn2Iter = new RnsIterator(in2.coeff, in2.offset + (in2Index - j) * n * baseSize, n, in2.k);

            for (int k = 0; k < baseSize; k++) {
                CoeffIterator temp = CoeffIterator.createZeroCoeff(n);
                // c_1 mod q_i, c_2 mod q_i in NTT form, compute c_1 * c_2 mod q_i
                PolyArithmeticSmallMod.dyadicProductCoeffMod(
                        shiftedIn1Iter.rnsIterators[j].coeffIterators[k],
                        shiftedReversedIn2Iter.coeffIterators[k],
                        n,
                        base[k],
                        temp
                );
                PolyArithmeticSmallMod.addPolyCoeffMod(temp, shiftedOutIter.coeffIterators[k], n, base[k], shiftedOutIter.coeffIterators[k]);
            }
        }
    }
    ...
    private void bfvSquare(Ciphertext encrypted) {
        ...
        // Allocate space for a base q output of behz_extend_base_convert_to_ntt for encrypted1
        PolyIterator encryptedQ = PolyIterator.createZeroPoly(encryptedSize, coeffCount, baseQSize);

        // Allocate space for a base Bsk output of behz_extend_base_convert_to_ntt for encrypted1
        PolyIterator encryptedBsk = PolyIterator.createZeroPoly(encryptedSize, coeffCount, baseBskSize);
        ...
        for (int i = 0; i < encryptedSize; i++) {
            ...
            PolyCore.setPoly(
                    ...
                    encryptedQ.coeff, i * coeffCount * baseQSize
            );
            // Lazy reduction
            NttTool.nttNegacyclicHarveyLazyRns(encryptedQ.rnsIterators[i], baseQSize, baseQNttTables);

            // Allocate temporary space for a polynomial in base {B_sk, {m_tilde}}
            RnsIterator temp = RnsIterator.createZeroRns(coeffCount, baseBskMTildeSize);
        ...
        behzCiphertextSquare(encryptedQ, baseQ, tempDestinationQ);
        // behz_ciphertext_square(encrypted_Bsk, base_Bsk, base_Bsk_size, temp_dest_Bsk);
        behzCiphertextSquare(encryptedBsk, baseBsk, tempDestinationBsk);
        ...
        for (int i = 0; i < destinationSize; i++) {
            ...
            PolyArithmeticSmallMod.multiplyPolyScalarCoeffModRns(
                    tempDestinationBsk.rnsIterators[i], baseBskSize,
                    plainModulus, baseBsk,
                    tempQBsk.subRnsIterator(baseQSize)
            );
            ...
            // Step (8): use Shenoy-Kumaresan method to convert the result to base q and write to encrypted1
            rnsTool.fastBConvSkRnsIter(tempBsk, encryptedPoly.rnsIterators[i]);
        }
    }
    ...
    private void behzCiphertextSquare(PolyIterator in, Modulus[] baseQ, PolyIterator destination) {
        ...
        PolyArithmeticSmallMod.dyadicProductCoeffModRns(
                in.rnsIterators[0], in.rnsIterators[0], baseQSize,
                baseQ,
                destination.rnsIterators[0]
        );

        // compute 2 * c0 * c1
        PolyArithmeticSmallMod.dyadicProductCoeffModRns(
                in.rnsIterators[0], in.rnsIterators[1], baseQSize,
                baseQ,
                destination.rnsIterators[1]
        );
        ...
        // compute c1^2
        PolyArithmeticSmallMod.dyadicProductCoeffModRns(
                in.rnsIterators[1], in.rnsIterators[1], baseQSize,
                baseQ,
                destination.rnsIterators[2]
        );
    ...
    private void relinearizeInternal(Ciphertext encrypted, RelinKeys relinKeys, int destinationSize) {
        ...
        int encryptedIterIndex = (encryptedSize - 1) * encrypted.polyModulusDegree() * encrypted.getCoeffModulusSize();
        RnsIterator encryptedIter = new RnsIterator(encrypted.data(), encryptedIterIndex, encrypted.polyModulusDegree(), encrypted.getCoeffModulusSize());

        for (int i = 0; i < reLinsNeeded; i++) {
            switchKeyInplace(
                    encrypted, encryptedIter, relinKeys, RelinKeys.getIndex(encryptedSize - 1 - i)
            );
        }
    ...
    private void modSwitchScaleToNext(Ciphertext encrypted, Ciphertext destination) {
        ...
        PolyIterator encryptedCopyPoly = PolyIterator.fromCiphertext(encryptedCopy);

        switch (nextParms.scheme()) {
            case BFV:
                for (int i = 0; i < encryptedSize; i++) {
                    rnsTool.divideAndRoundQLastInplace(encryptedCopyPoly.rnsIterators[i]);
                }
                break;
            case CKKS:
                // TODO: implement CKKS
    ...
    public void addPlainInplace(Ciphertext encrypted, Plaintext plain) {
        ...
        switch (parms.scheme()) {
            case BFV:
                RnsIterator c0 = new RnsIterator(encrypted.data(), encrypted.getPolyOffset(0), coeffCount, encrypted.getCoeffModulusSize());
    ...
    private void multiplyPlainNormal(Ciphertext encrypted, Plaintext plain) {
        ...
        */
        ...
        if (plainNonZeroCoeffCount == 1) {
            ...
            if (plain.at(monoExponent) >= plainUpperHalfThreshold) {
                ...
                if (!contextData.qualifiers().isUsingFastPlainLift()) {
                    ...
                    CoeffIterator temp = CoeffIterator.createZeroCoeff(coeffModulusSize);
                    ...
                    UintArithmetic.addUint(plainUpperHalfIncrement, coeffModulusSize, plain.at(monoExponent), temp.getCoeff());
                    contextData.rnsTool().baseQ().decompose(temp.getCoeff());
                    PolyArithmeticSmallMod.negacyclicMultiplyPolyMonoCoeffModPoly(
                            encryptedPoly, encryptedSize,
        ...
        if (!contextData.qualifiers().isUsingFastPlainLift()) {
            ...
            StrideIterator tempIter = new StrideIterator(temp, 0, coeffModulusSize);

            for (int i = 0; i < plainCoeffCount; i++) {
                long plainValue = plain.at(i);
                if (plainValue >= plainUpperHalfThreshold) {
                    UintArithmetic.addUint(plainUpperHalfIncrement, 0, coeffModulusSize, plainValue, tempIter.coeff, i * coeffModulusSize);
                } else {
                    tempIter.setCoefficient(i, plainValue);
                }
            }
        ...
        RnsIterator tempIter = new RnsIterator(temp, 0, coeffCount, coeffModulusSize);
        NttTool.nttNegacyclicHarveyRns(tempIter, coeffModulusSize, nttTables);

        for (int i = 0; i < encryptedSize; i++) {
            for (int j = 0; j < coeffModulusSize; j++) {
                // Lazy Reduction
                NttTool.nttNegacyclicHarveyLazy(encryptedPoly.rnsIterators[i].coeffIterators[j], nttTables[j]);
                PolyArithmeticSmallMod.dyadicProductCoeffMod(
                        encryptedPoly.rnsIterators[i].coeffIterators[j],
                        tempIter.coeffIterators[j],
                        coeffCount, coeffModulus[j],
                        encryptedPoly.rnsIterators[i].coeffIterators[j]
                );
                NttTool.inverseNttNegacyclicHarvey(encryptedPoly.rnsIterators[i].coeffIterators[j], nttTables[j]);
    ...
    private void multiplyPlainNtt(Ciphertext encryptedNtt, Plaintext plainNtt) {
        ...
        RnsIterator plainNttIter = new RnsIterator(plainNtt.getData(), 0, coeffCount, coeffModulusSize);
        PolyIterator encryptedNttPoly = PolyIterator.fromCiphertext(encryptedNtt);

        for (int i = 0; i < encryptedNttSize; i++) {
            PolyArithmeticSmallMod.dyadicProductCoeffModRns(
                    encryptedNttPoly.rnsIterators[i], plainNttIter,
                    coeffModulusSize, coeffModulus,
                    encryptedNttPoly.rnsIterators[i]
            );
        }
        // Set the scale
...
//            long[] temp = new long[coeffCount * coeffModulusSize];
            // 注意这里的 RnsIter 它的 N = k , k = N
            // 一共 coeffCount 个 CoeffIter, 每一个长度为 coeffModulusSize
            RnsIterator temp = RnsIterator.createZeroRns(coeffModulusSize, coeffCount);

            for (int i = 0; i < plainCoeffCount; i++) {
                long plainValue = plain.at(i);
                if (plainValue >= plainUpperHalfThreshold) {
                    UintArithmetic.addUint(plainUpperHalfIncrement, 0, coeffModulusSize, plainValue, temp.coeff, i * coeffModulusSize);
                } else {
                    temp.coeffIterators[i].setCoefficient(0, plainValue);
//                    temp[i * coeffModulusSize] = plainValue;
                }
            }
            contextData.rnsTool().baseQ().decomposeArray(temp.coeff, coeffCount);

            // Copy data back to plain
            System.arraycopy(temp.coeff, 0, plain.getData(), 0, coeffCount * coeffModulusSize);
        } else {
            // Note that in this case plain_upper_half_increment holds its value in RNS form modulo the coeff_modulus
            // primes.
        ...
        RnsIterator plainIter = new RnsIterator(plain.getData(), 0, coeffCount, coeffModulusSize);
        // Transform to NTT domain
//        NttTool.nttNegacyclicHarveyRns(plain.getData(), coeffCount, coeffModulusSize, nttTables);
        NttTool.nttNegacyclicHarveyRns(plainIter, coeffModulusSize, nttTables);
    ...
    public void applyGaloisInplace(Ciphertext encrypted, int galoisElt, GaloisKeys galoisKeys) {
        ...
        RnsIterator temp = RnsIterator.createZeroRns(coeffCount, coeffModulusSize);
        ...
        if (parms.scheme().equals(SchemeType.BFV) || parms.scheme().equals(SchemeType.BGV)) {
            ...
            RnsIterator c0 = new RnsIterator(encrypted.data(), 0, encrypted.polyModulusDegree(), coeffModulusSize);
            ...
            PolyCore.setPoly(temp.coeff, coeffCount, coeffModulusSize, c0.coeff);

            // Next transform encrypted.data(1)
            RnsIterator c1 = new RnsIterator(encrypted.data(), coeffCount * coeffModulusSize, coeffCount, coeffModulusSize);
            galoisTool.applyGaloisRnsIter(
                    c1, coeffModulusSize,
                    galoisElt, coeffModulus,
                    temp
        ...
        switchKeyInplace(encrypted, temp, galoisKeys, GaloisKeys.getIndex(galoisElt));
    ...
    private void switchKeyInplace(Ciphertext encrypted, RnsIterator targetIter, KswitchKeys kswitchKeys, int keySwitchKeysIndex) {
        ...
        assert targetIter.n == coeffCount;
        assert targetIter.k == decompModulusSize;
        ...
        RnsIterator tTarget = RnsIterator.createZeroRns(coeffCount, decompModulusSize);
        UintCore.setUint(targetIter.coeff,
                targetIter.offset,
        ...
        if (scheme.equals(SchemeType.CKKS)) {
            NttTool.inverseNttNegacyclicHarveyRns(tTarget, decompModulusSize, keyNttTables);
        }

        // Temporary result
        // tPolyProd 不能直接在这里定义为 一个 PolyIter
        ...
        for (int i = 0; i < rnsModulusSize; i++) {
            ...
            for (int j = 0; j < decompModulusSize; j++) {
                CoeffIterator tNtt = CoeffIterator.createZeroCoeff(coeffCount);
                CoeffIterator tOperand;

                // RNS-NTT form exists in input
                if ((scheme.equals(SchemeType.CKKS)) && (i == j)) {
                    // TODO: implement CKKS, tOperandPos = j * coeffCount may be incorrect
                    tOperand = targetIter.coeffIterators[j];
                } else {
                    ...
                    if (keyModulus[j].value() <= keyModulus[keyIndex].value()) {
                        ...
                        UintCore.setUint(tTarget.coeff, j * coeffCount, coeffCount, tNtt.getCoeff(), 0, coeffCount);
                    } else {
                        // Perform RNS conversion (modular reduction)
                        PolyArithmeticSmallMod.moduloPolyCoeff(tTarget.coeffIterators[j],
                                coeffCount,
                                keyModulus[keyIndex],
                                tNtt
                        );
            ...
            // 注意这里的 i * coeffCount 是指向 PolyIter 下的每一个 RnsIter 的 第 i 个 coeffIter
            // 它并不是来定位 RnsIter 的，所以我们在这里构造 PolyIter 的时候，不需要考虑 offset
            // 在后续使用定位到 CoeffIter 的时候再考虑 offset 即可
            PolyIterator tPolyProdIter = new PolyIterator(tPolyProd, 0, coeffCount, rnsModulusSize);

            // Final modular reduction
            for (int k = 0; k < keyComponentCount; k++) {
                if (lazyReductionCounter == lazyReductionSummandBound) {
                    for (int l = 0; l < coeffCount; l++) {
                        // 注意这里要取第 i 个 CoeffIter, 正好对应了前面的 i * coeffCount
                        tPolyProdIter.rnsIterators[k].coeffIterators[i].setCoefficient(
                                l,
        ...
        for (int i = 0; i < keyComponentCount; i++) {
            if (scheme.equals(SchemeType.BGV)) {
                ...
                CoeffIterator tLast = tPolyProdIter.rnsIterators[i].coeffIterators[decompModulusSize];
                NttTool.inverseNttNegacyclicHarveyLazy(tLast, keyNttTables[keyModulusSize - 1]);
                ...
                for (int j = 0; j < coeffCount; j++) {
                    tLast.setCoefficient(
                            j,
                            UintArithmeticSmallMod.barrettReduce64(
                                    tLast.getCoefficient(j) + qkHalf, keyModulus[keyModulusSize - 1]
                ...
                for (int j = 0; j < decompModulusSize; j++) {
                    ...
                    if (qk > qi) {
                        ...
                        PolyArithmeticSmallMod.moduloPolyCoeff(
                                tLast, coeffCount, keyModulus[j], tNtt
                        );
                    } else {
                        UintCore.setUint(tLast.getCoeff(), tLast.getOffset(), coeffCount, tNtt.getCoeff(), 0, coeffCount);
                    ...
                    for (int k = 0; k < coeffCount; k++) {
                        tNtt.setCoefficient(k, tNtt.getCoefficient(k) + fix);
                    }
                    // 用来定位 get<0, 1>(J), tPolyProd 中 第 i 个 RnsIter 中的 第 j 个 CoeffIter
                    CoeffIterator zeroOneJ = tPolyProdIter.rnsIterators[i].coeffIterators[j];
                    long qiLazy = qi << 1;
                    if (scheme.equals(SchemeType.CKKS)) {
                        // TODO: implement CKKS
                        throw new IllegalArgumentException("unsupported CKKS");
                    } else if (scheme.equals(SchemeType.BFV)) {
                        NttTool.inverseNttNegacyclicHarveyLazy(zeroOneJ, keyNttTables[j]);
                    }
                    // ((ct mod qi) - (ct mod qk)) mod qi with output in [0, 2 * qi_lazy)
                    for (int k = 0; k < coeffCount; k++) {
                        zeroOneJ.setCoefficient(
                                k,
                                zeroOneJ.getCoefficient(k) + qiLazy - tNtt.getCoefficient(k)
                        );
                    }

                    // qk^(-1) * ((ct mod qi) - (ct mod qk)) mod qi
                    PolyArithmeticSmallMod.multiplyPolyScalarCoeffMod(
                            zeroOneJ, coeffCount, modSwitchFactors[j],
                            keyModulus[j],
                            zeroOneJ);
                    CoeffIterator zeroZeroJ = encryptedPoly.rnsIterators[i].coeffIterators[j];
                    PolyArithmeticSmallMod.addPolyCoeffMod(
                            zeroOneJ, zeroZeroJ,
