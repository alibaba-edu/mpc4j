...
import edu.alibaba.mpc4j.crypto.swhe.seal.PublicKey;
import edu.alibaba.mpc4j.crypto.swhe.seal.SecretKey;
...
import edu.alibaba.mpc4j.crypto.swhe.seal.rand.ClippedNormalDistribution;
import edu.alibaba.mpc4j.crypto.swhe.seal.rand.UniformRandomGenerator;
import edu.alibaba.mpc4j.crypto.swhe.seal.rand.UniformRandomGeneratorFactory;
import edu.alibaba.mpc4j.crypto.swhe.seal.rand.UniformRandomGeneratorInfo;
...
public class RingLwe {
    ...
    public static void samplePolyTernary(UniformRandomGenerator prng, EncryptionParameters parms,
        ...
        for (int j = 0; j < coeffCount; j++) {
            ...
            StrideIterator temp = new StrideIterator(destination, offset + j, coeffCount);
            for (int i = 0; i < coeffModulusSize; i++) {
                temp.setCoefficient(i, rand + (flag & coeffModulus[i].value()) - 1);
            }
        }
    }
    ...
    public static void samplePolyNormal(UniformRandomGenerator prng, EncryptionParameters parms,
        ...
        for (int j = 0; j < coeffCount; j++) {
            ...
            StrideIterator temp = new StrideIterator(destination, offset + j, coeffCount);
            for (int i = 0; i < coeffModulusSize; i++) {
                temp.setCoefficient(i, noise + (flag & coeffModulus[i].value()));
            }
        }
    }
    ...
    public static void samplePolyCbd(UniformRandomGenerator prng, EncryptionParameters parms,
        ...
        for (int j = 0; j < coeffCount; j++) {
            ...
            StrideIterator temp = new StrideIterator(destination, offset + j, coeffCount);
            for (int i = 0; i < coeffModulusSize; i++) {
                temp.setCoefficient(i, (long) noise + (flag & coeffModulus[i].value()));
            }
        }
    }
    ...
    public static void encryptZeroAsymmetric(PublicKey publicKey, SealContext context, ParmsId parmsId,
        ...
        for (int j = 0; j < encryptedSize; j++) {
            ...
            RnsIterator gaussianIter = new RnsIterator(u, 0, coeffCount, coeffModulusSize);
            ...
            RnsIterator dstIter = new RnsIterator(destination.data(), destination.getPolyOffset(j), coeffCount, coeffModulusSize);

            PolyArithmeticSmallMod.addPolyCoeffModRns(gaussianIter, dstIter, coeffModulusSize, coeffModulus, dstIter);
        }
    }
