 ...
 * <a href="https://github.com/microsoft/SEAL/blob/v4.0.0/native/src/seal/util/rns.h#L190">
 * RNSTool in rns.h
 * </a>.
 *
...
public class RnsTool {
    ...
    public void decryptScaleAndRound(RnsIterator input, CoeffIterator destination) {
        ...
        assert destination.getN() == this.coeffCount;
...
//        long[] temp = PolyCore.allocateZeroPolyArray(baseQSize, input.n, 1);
        RnsIterator temp = RnsIterator.createZeroRns(input.n, baseQSize);

        for (int i = 0; i < baseQSize; i++) {
            // compute temp = |γt · ct(s)|_{q_1, ..., q_k}
            PolyArithmeticSmallMod.multiplyPolyScalarCoeffMod(input.coeffIterators[i], input.n, prodTGammaModQ[i], baseQ.getBase(i), temp.coeffIterators[i]);
...
//        long[] tempTGammaRns = PolyCore.allocateZeroPolyArray(baseTGammaSize, inputN, 1);
        ...
        RnsIterator tempTGammaRns = RnsIterator.createZeroRns(input.n, baseTGammaSize);
        ...
        for (int i = 0; i < baseTGammaSize; i++) {
            PolyArithmeticSmallMod.multiplyPolyScalarCoeffMod(
                tempTGammaRns.coeffIterators[i], input.n,
                negInvQModTGamma[i], baseTGamma.getBase(i),
                tempTGammaRns.coeffIterators[i]
            );
        }
        ...
        for (int i = 0; i < input.n; i++) {
            if (tempTGammaRns.coeffIterators[1].getCoefficient(i) > gammaDiv2) {
                // the coefficient is in [q/2, q).
                // compute |s^(t) + (γ - |s^(γ)|_{γ})|_t instead of |s^t - (|s^(γ)|_{γ} - γ)|_t
                destination.setCoefficient(
                    i,
                    UintArithmeticSmallMod.addUintMod(
                        tempTGammaRns.coeffIterators[0].getCoefficient(i),
                        UintArithmeticSmallMod.barrettReduce64(gamma.value() - tempTGammaRns.coeffIterators[1].getCoefficient(i), t),
                        t
                    ));
            } else {
                ...
                destination.setCoefficient(
                    ...
                    UintArithmeticSmallMod.subUintMod(
                        tempTGammaRns.coeffIterators[0].getCoefficient(i),
                        UintArithmeticSmallMod.barrettReduce64(tempTGammaRns.coeffIterators[1].getCoefficient(i), t),
                        t
                    ));
            }
            // step 5(2): m^(t) ← [m^(t) × |γ^{-1}|_{t}]_t, therefore, m^(t) ← [[(s^(t) - ~s^(γ))]_t × |γ^{-1}|_{t}]_t
            if (destination.getCoefficient(i) != 0) {
                destination.setCoefficient(
                    i,
                    UintArithmeticSmallMod.multiplyUintMod(destination.getCoefficient(i), invGammaModT, t)
                );
            }
        }
    }
    ...
    public void fastBConvMTildeRnsIter(RnsIterator input, RnsIterator destination) {
        ...
        RnsIterator temp = RnsIterator.createZeroRns(coeffCount, baseQSize);
        // (input * m_tilde) mod q
        PolyArithmeticSmallMod.multiplyPolyScalarCoeffModRns(input, input.k, mTilde.value(), baseQ.getBase(), temp);
        // Now convert to B_sk, i.e., FastBcov([input * m_tilde]_q, q, {B_sk})
        RnsIterator tempOut = RnsIterator.createZeroRns(coeffCount, baseBskSize);

        baseQToBskConv.fastConvertArrayRnsIter(temp, tempOut);
        // Finally, convert to {m_tilde}, i.e., FastBcov([input * m_tilde]_q, q, {m_tilde})
        RnsIterator tempOut2 = RnsIterator.createZeroRns(coeffCount, 1);
        ...
        System.arraycopy(tempOut.coeff, 0, destination.coeff, destination.offset, tempOut.coeff.length);
        System.arraycopy(tempOut2.coeff, 0, destination.coeff, destination.offset + tempOut.coeff.length, tempOut2.coeff.length);
    }
    ...
    public void smMrqRnsIter(RnsIterator input, RnsIterator destination) {
        ...
        CoeffIterator inputMTilde = input.coeffIterators[baseBskSize];
        ...
        CoeffIterator rMTilde = CoeffIterator.createZeroCoeff(coeffCount);
        ...
        for (int i = 0; i < baseBskSize; i++) {
            ...
            for (int j = 0; j < coeffCount; j++) {
                long temp = rMTilde.getCoefficient(j);
                ...
                long c = UintArithmeticSmallMod.multiplyAddUintMod(
                    temp, prodQModBskElt, input.coeffIterators[i].getCoefficient(j), baseBsk.getBase(i)
                );
                // c'_m ← |c · {m_tilde}^(-1)|_m = |(c''_m + q · r_{m_tilde}) · {m_tilde}^(-1)|_{m}
                destination.coeffIterators[i].setCoefficient(
                    j,
            ...
            for (int j = 0; j < coeffCount; j++) {
                ...
                destination.coeffIterators[i].setCoefficient(
                    ...
                    UintArithmeticSmallMod.multiplyUintMod(
                        input.coeffIterators[baseQSize + i].getCoefficient(j)
                            + (baseBsk.getBase(i).value() - destination.coeffIterators[i].getCoefficient(j)),
                        invProdQModBsk[i],
                        baseBsk.getBase(i)
                    ));
            }
    ...
    public void fastBConvSkRnsIter(RnsIterator input, RnsIterator destination) {
        ...
        RnsIterator temp = RnsIterator.createZeroRns(coeffCount, 1);
        // FastBonv(x, B, {m_sk})
        ...
        CoeffIterator alphaSk = CoeffIterator.createZeroCoeff(coeffCount);
        // we first compute |(FastBonv(x, B, {m_sk}) - x_{sk}) · B^{-1}|_{m_{sk}}
        for (int i = 0; i < coeffCount; i++) {
            alphaSk.setCoefficient(
                i,
                UintArithmeticSmallMod.multiplyUintMod(
                    temp.coeffIterators[0].getCoefficient(i) + (mSk.value() - input.coeffIterators[baseBSize].getCoefficient(i)),
                    invProdBModMsk, mSk
                ));
        ...
        for (int i = 0; i < baseQSize; i++) {
            ...
            for (int j = 0; j < coeffCount; j++) {
                ...
                if (alphaSk.getCoefficient(j) > mSkDiv2) {
                    destination.coeffIterators[i].setCoefficient(
                        j,
                        UintArithmeticSmallMod.multiplyAddUintMod(
                            UintArithmeticSmallMod.negateUintMod(alphaSk.getCoefficient(j), mSk), prodBModQElt,
                            destination.coeffIterators[i].getCoefficient(j), baseQ.getBase(i)
                        ));
                } else {
                    ...
                    destination.coeffIterators[i].setCoefficient(
                        j,
                        UintArithmeticSmallMod.multiplyAddUintMod(
                            alphaSk.getCoefficient(j), negProdBModQElt, destination.coeffIterators[i].getCoefficient(j),
                            baseQ.getBase(i)
                        ));
                }
            }
...
//        int lastInputIndex = pos + (baseQSize - 1) * n;
        CoeffIterator lastInput = poly.coeffIterators[baseQSize - 1];
        ...
        CoeffIterator temp = CoeffIterator.createZeroCoeff(coeffCount);
        for (int i = 0; i < baseQSize - 1; i++) {
            // (ct mod q_k) mod q_i
            ...
            PolyArithmeticSmallMod.subPolyCoeffMod(poly.coeffIterators[i], temp, coeffCount, baseQ.getBase(i), poly.coeffIterators[i]);
            // (ct mod q_i) = q_k^(-1) * ((ct mod q_i) - (ct mod q_k)) mod q_i,
            PolyArithmeticSmallMod.multiplyPolyScalarCoeffMod(poly.coeffIterators[i], coeffCount, invQLastModQ[i], baseQ.getBase(i), poly.coeffIterators[i]);
        }
    }
    ...
    public void divideAndRoundQLastNttInplace(RnsIterator poly, NttTables[] rnsNttTables) {
        ...
        for (int i = 0; i < baseQSize - 1; i++) {
            ...
            if (baseQ.getBase(i).value() < lastModulus.value()) {
                ...
                System.arraycopy(lastInput.getCoeff(), lastInput.getOffset(), temp.getCoeff(), temp.getOffset(), coeffCount);
            }
            ...
            for (int j = 0; j < coeffCount; j++) {
                temp.setCoefficient(j, temp.getCoefficient(j) + negHalfMod);
            }
            long qiLazy;
            if (Constants.SEAL_USER_MOD_BIT_COUNT_MAX <= 60) {
                ...
                for (int j = 0; j < coeffCount; j++) {
                    long subValue = qiLazy & (temp.getCoefficient(j) >= qiLazy ? -1 : 0);
                    temp.setCoefficient(
                        j,
                        temp.getCoefficient(j) - subValue
                    );
                }
            }
            ...
            for (int j = 0; j < coeffCount; j++) {
                ...
                poly.coeffIterators[i].setCoefficient(j,
                    poly.coeffIterators[i].getCoefficient(j) + (qiLazy - temp.getCoefficient(j))
                );
            }
            // q_k^(-1) * ct' mod q_i
            PolyArithmeticSmallMod.multiplyPolyScalarCoeffMod(
                poly.coeffIterators[i], coeffCount, invQLastModQ[i], baseQ.getBase(i), poly.coeffIterators[i]
            );
        }
    }
