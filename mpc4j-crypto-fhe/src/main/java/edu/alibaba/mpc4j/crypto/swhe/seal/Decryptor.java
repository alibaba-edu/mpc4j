...
public class Decryptor {
    ...
    private void bfvDecrypt(Ciphertext encrypted, Plaintext destination) {
        ...
        RnsIterator tempDestModQ = RnsIterator.createZeroRns(coeffCount, coeffModulusSize);
        ...
        CoeffIterator destinationCoeff = new CoeffIterator(destination.getData(),0, coeffCount);
     ...
     */
    private void dotProductCtSkArray(Ciphertext encrypted, RnsIterator destination) {
        ContextData contextData = context.getContextData(encrypted.parmsId());
        EncryptionParameters parms = contextData.parms();
        Modulus[] coeffModulus = parms.coeffModulus();
        int coeffCount = parms.polyModulusDegree();
        ...
        if (encryptedSize == 2) {
            ...
            RnsIterator secretKeyArrayRns = new RnsIterator(secretKeyArray, 0, coeffCount, coeffModulusSize);
            RnsIterator c0 = new RnsIterator(encrypted.data(), encrypted.getPolyOffset(0), coeffCount, coeffModulusSize);
            RnsIterator c1 = new RnsIterator(encrypted.data(), encrypted.getPolyOffset(1), coeffCount, coeffModulusSize);

            if (isNttForm) {
                for (int i = 0; i < coeffModulusSize; i++) {
                    // put < c_1 * s > mod q in destination
                    PolyArithmeticSmallMod.dyadicProductCoeffMod(c1.coeffIterators[i], secretKeyArrayRns.coeffIterators[i],
                            coeffCount,
                            coeffModulus[i],
                            destination.coeffIterators[i]
                    );
                    ...
                    PolyArithmeticSmallMod.addPolyCoeffMod(
                            destination.coeffIterators[i], c0.coeffIterators[i],
                            coeffCount, coeffModulus[i], destination.coeffIterators[i]
                    );
                }
            } else {
                for (int i = 0; i < coeffModulusSize; i++) {

                    System.arraycopy(c1.coeffIterators[i].getCoeff(), c1.coeffIterators[i].getOffset(),
                            destination.coeffIterators[i].getCoeff(), destination.coeffIterators[i].getOffset(),
                            coeffCount
                    );

                    // transform c_1 to NTT form
                    NttTool.nttNegacyclicHarveyLazy(destination.coeffIterators[i], nttTables[i]);
                    // put < c_1 * s > mod q in destination
                    PolyArithmeticSmallMod.dyadicProductCoeffMod(
                            destination.coeffIterators[i], secretKeyArrayRns.coeffIterators[i],
                            coeffCount, coeffModulus[i], destination.coeffIterators[i]
                    );
                    // transform back
                    NttTool.inverseNttNegacyclicHarvey(destination.coeffIterators[i], nttTables[i]);
            ...
            // Now do the dot product of encrypted_copy and the secret key array using NTT.
            // The secret key powers are already NTT transformed.
            PolyIterator encryptedCopy = PolyIterator.createZeroPoly(encryptedSize - 1, coeffCount, coeffModulusSize);
            ...
            PolyIterator secretKeyArrayPoly = new PolyIterator(secretKeyArray, 0, coeffCount, keyCoeffModulusSize);

            // Compute dyadic product with secret power array: c1 * s, c2 * s^2 ...
            for (int i = 0; i < (encryptedSize - 1); i++) {
                PolyArithmeticSmallMod.dyadicProductCoeffModRns(
                        encryptedCopy.rnsIterators[i],
                        secretKeyArrayPoly.rnsIterators[i],
                        coeffModulusSize,
                        coeffModulus,
                        encryptedCopy.rnsIterators[i]
                );
            ...
            for (int i = 0; i < (encryptedSize - 1); i++) {
                PolyArithmeticSmallMod.addPolyCoeffModRns(destination, encryptedCopy.rnsIterators[i], coeffModulusSize, coeffModulus, destination);
            }
            if (!isNttForm) {
            ...
            RnsIterator c0 = new RnsIterator(encrypted.data(), encrypted.getPolyOffset(0), coeffCount, coeffModulusSize);
            // add
            PolyArithmeticSmallMod.addPolyCoeffModRns(destination, c0, coeffModulusSize, coeffModulus, destination);
        }
    }
    ...
    public int invariantNoiseBudget(Ciphertext encrypted) {
        ...
        RnsIterator noisePoly = RnsIterator.createZeroRns(coeffCount, coeffModulusSize);
        ...
        if (scheme.equals(SchemeType.BFV)) {
            PolyArithmeticSmallMod.multiplyPolyScalarCoeffModRns(noisePoly, coeffModulusSize, plainModulus.value(), coeffModulus, noisePoly);
        }

        // CRT-compose the noise
        contextData.rnsTool().baseQ().composeArray(noisePoly.coeff, coeffCount);

        // Next we compute the infinity norm mod parms.coeff_modulus()
        StrideIterator wideNoisePoly = new StrideIterator(noisePoly.coeff, 0, coeffModulusSize);
        polyInftyNormCoeffModStrideIter(wideNoisePoly, coeffCount, contextData.totalCoeffModulus(), norm);
    ...
    private void polyInftyNormCoeffModStrideIter(StrideIterator poly, int coeffCount, long[] modulus,
                                                 long[] result) {

        int coeffUint64Count = poly.stride;
        ...
        for (int i = 0; i < coeffCount; i++) {
            if (UintCore.isGreaterThanOrEqualUint(poly.coeff, i * coeffUint64Count, modulusNegThreshold, 0, coeffUint64Count)) {
                UintArithmetic.subUint(modulus, 0, poly.coeff, i * coeffUint64Count, coeffUint64Count, coeffAbsValue);
            } else {
                UintCore.setUint(poly.coeff, i * coeffUint64Count, coeffUint64Count, coeffAbsValue, 0, coeffUint64Count);
            }
