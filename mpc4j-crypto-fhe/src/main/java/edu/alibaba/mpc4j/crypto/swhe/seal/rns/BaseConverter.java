 ...
 * <a href="https://github.com/microsoft/SEAL/blob/v4.0.0/native/src/seal/util/rns.h#L129">
 * BaseConverter in rns.h
 * </a>.
 *
...
public class BaseConverter {
    ...
    public void fastConvert(CoeffIterator in, CoeffIterator out) {
        ...
        // 注意这里的 CoeffIterator 其并不表示多项式的N个系数，就是表示值 x \in Z_q, 在 InBase q = [q1, q2, ..., qk] 下的表示
        assert in.getN() == inBase.size();
        assert out.getN() == outBase.size();
        // temp = x_i * ~{q_i} mod q_i
        long[] temp = IntStream.range(0, inBase.size())
                .mapToLong(i ->
                        UintArithmeticSmallMod.multiplyUintMod(
                                in.getCoefficient(i), inBase.getInvPuncturedProdModBaseArray(i), inBase.getBase(i)
                        )
                )
                .toArray();
        // x'_i = Σ_{i = 1}^{k} (x_i * ~{q_i} mod q_i) * q_i^* mod m_j
        for (int i = 0; i < outBase.size(); i++) {
            out.setCoefficient(i, UintArithmeticSmallMod.dotProductMod(temp, baseChangeMatrix[i], inBase.size(), outBase.getBase(i)));
        }
    }
            ...
            if (inBase.getInvPuncturedProdModBaseArray(i).operand == 1) {
                for (int j = 0; j < in.n; j++) {
                    temp[j][i] = UintArithmeticSmallMod.barrettReduce64(
                            in.coeffIterators[i].getCoefficient(j), inBase.getBase(i)
//                            in[inPos + i * inN + j], inBase.getBase(i)
                ...
                for (int j = 0; j < in.n; j++) {
                    temp[j][i] = UintArithmeticSmallMod.multiplyUintMod(
                            in.coeffIterators[i].getCoefficient(j), inBase.getInvPuncturedProdModBaseArray(i), inBase.getBase(i)
//                            in[inPos + i * inN + j], inBase.getInvPuncturedProdModBaseArray(i), inBase.getBase(i)
        ...
        for (int i = 0; i < outBase.size(); i++) {
            for (int j = 0; j < in.n; j++) {
                out.coeffIterators[i].setCoefficient(j, UintArithmeticSmallMod.dotProductMod(
...
//                );
    ...
    public void exactConvertArray(RnsIterator in, CoeffIterator out) {
        assert in.n == out.getN();
        assert in.k == inBase.size();
        long[][] inCoeffs = RnsIterator.rnsTo2dArray(in.coeff, in.n, in.k);
        ...
        for (int i = 0; i < in.n; i++) {
            out.setCoefficient(i, exactConvert(inColumns[i]));
        }
    }
