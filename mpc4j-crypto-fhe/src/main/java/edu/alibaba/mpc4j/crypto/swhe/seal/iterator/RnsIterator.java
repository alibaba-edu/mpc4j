 ...
 * <a href="https://github.com/microsoft/SEAL/blob/v4.0.0/native/src/seal/util/iterator.h#L951">
 * RNSIter in iterator.h
 * </a>.
 *
 * @author Weiran Liu
 * @date 2023/11/5
...
public class RnsIterator {
     ...
     * @return an 1D-array RNS representations with all coefficients initialized as 0.
     */
    public static RnsIterator createZeroRns(int n, int k) {
        assert n > 0;
        assert k > 0;
    ...
    public RnsIterator subRnsIterator(int fromIndex, int toIndex) {
        ...
        assert toIndex < k;
        return new RnsIterator(coeff, offset + fromIndex * n, n, toIndex - fromIndex + 1);
    }
    ...
    public RnsIterator subRnsIterator(int fromIndex) {
        ...
        return new RnsIterator(coeff, offset + fromIndex * n, n, k - fromIndex);
    }
