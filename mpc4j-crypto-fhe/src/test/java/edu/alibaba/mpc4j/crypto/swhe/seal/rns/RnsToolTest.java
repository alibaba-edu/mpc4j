...
import edu.alibaba.mpc4j.crypto.swhe.seal.zq.Numth;
...
public class RnsToolTest {
    ...
    @Test
    public void testFastBConvMTildeIterator() {
        ...
        RnsTool rnsTool;
        {
            ...
            RnsIterator inRns = new RnsIterator(in, n, inK);
            RnsIterator outRns = new RnsIterator(out, n, outK);
            rnsTool.fastBConvMTildeRnsIter(inRns, outRns);
            for (long val : outRns.coeff) {
                Assert.assertEquals(0, val);
            }

            in[0] = 1;
            in[1] = 2;
            inRns = new RnsIterator(in, n, inK);
            rnsTool.fastBConvMTildeRnsIter(inRns, outRns);
            ...
            Assert.assertEquals(temp % (rnsTool.baseBskMTilde().getBase(0).value()), outRns.coeff[0]);
            Assert.assertEquals(temp2 % (rnsTool.baseBskMTilde().getBase(0).value()), outRns.coeff[1]);
            Assert.assertEquals(temp % (rnsTool.baseBskMTilde().getBase(1).value()), outRns.coeff[2]);
            Assert.assertEquals(temp2 % (rnsTool.baseBskMTilde().getBase(1).value()), outRns.coeff[3]);
            Assert.assertEquals(temp % (rnsTool.baseBskMTilde().getBase(2).value()), outRns.coeff[4]);
            Assert.assertEquals(temp2 % (rnsTool.baseBskMTilde().getBase(2).value()), outRns.coeff[5]);
        }

        {
            ...
            Assert.assertEquals(temp % (rnsTool.baseBskMTilde().getBase(0).value()), outRns.coeff[0]);
            Assert.assertEquals(temp % (rnsTool.baseBskMTilde().getBase(0).value()), outRns.coeff[1]);
            Assert.assertEquals(temp % (rnsTool.baseBskMTilde().getBase(1).value()), outRns.coeff[2]);
            Assert.assertEquals(temp % (rnsTool.baseBskMTilde().getBase(1).value()), outRns.coeff[3]);
            Assert.assertEquals(temp % (rnsTool.baseBskMTilde().getBase(2).value()), outRns.coeff[4]);
            Assert.assertEquals(temp % (rnsTool.baseBskMTilde().getBase(2).value()), outRns.coeff[5]);
            Assert.assertEquals(temp % (rnsTool.baseBskMTilde().getBase(3).value()), outRns.coeff[6]);
            Assert.assertEquals(temp % (rnsTool.baseBskMTilde().getBase(3).value()), outRns.coeff[7]);
        }
    }
    ...
    public void testSmMrqIterator() {
        ...
        RnsTool rnsTool;
        {
            ...
            rnsTool.smMrqRnsIter(inRns, outRns);
            Assert.assertEquals(1, outRns.coeff[0]);
            Assert.assertEquals(2, outRns.coeff[1]);
            Assert.assertEquals(1, outRns.coeff[2]);
            Assert.assertEquals(2, outRns.coeff[3]);
...

        {
            ...
            Assert.assertEquals(1, outRns.coeff[2]);
            Assert.assertEquals(2, outRns.coeff[3]);
            Assert.assertEquals(1, outRns.coeff[4]);
            Assert.assertEquals(2, outRns.coeff[5]);
    ...
    @Test
    public void testFastFloorIterator() {
        ...
        RnsTool rnsTool;
        {
            ...
            rnsTool.fastFloorRnsIter(inRns, outRns);
            Assert.assertEquals(5, outRns.coeff[0]);
            Assert.assertEquals(1, outRns.coeff[1]);
            Assert.assertEquals(5, outRns.coeff[2]);
            Assert.assertEquals(1, outRns.coeff[3]);
...

        {
            ...
            inRns = new RnsIterator(in, n, inK);
            rnsTool.fastFloorRnsIter(inRns, outRns);
            Assert.assertTrue(Math.abs(1L - outRns.coeff[0]) <= 1);
            Assert.assertTrue(Math.abs(2L - outRns.coeff[1]) <= 1);
            Assert.assertTrue(Math.abs(1L - outRns.coeff[2]) <= 1);
            Assert.assertTrue(Math.abs(2L - outRns.coeff[3]) <= 1);
            Assert.assertTrue(Math.abs(1L - outRns.coeff[4]) <= 1);
            Assert.assertTrue(Math.abs(2L - outRns.coeff[5]) <= 1);
        }
    }
    ...
    public void testFastBConvSkIterator() {
        ...
        RnsTool rnsTool;
        {
            ...
            inRns = new RnsIterator(in, n, inK);
            rnsTool.fastBConvSkRnsIter(inRns, outRns);
            Assert.assertEquals(5 % 3, outRns.coeff[0]);
            Assert.assertEquals(6 % 3, outRns.coeff[1]);
        }
    ...
    @Test
    public void testExactScaleAndRoundIterator() {
        ...
        RnsIterator inRns = new RnsIterator(in, 0, coeffCount, rnsTool.baseBsk().size());
        CoeffIterator outCoeff = new CoeffIterator(out, 0, coeffCount);

        // 0 ---> scale and round must be 0
        rnsTool.decryptScaleAndRound(inRns, outCoeff);
        for (long o : outCoeff.getCoeff()) {
        ...
        inRns = new RnsIterator(in, 0, coeffCount, rnsTool.baseBsk().size());
        outCoeff = new CoeffIterator(out, 0, coeffCount);
        ...
        rnsTool.decryptScaleAndRound(inRns, outCoeff);
        Assert.assertEquals(2, outCoeff.getCoefficient(0));
        Assert.assertEquals(0, outCoeff.getCoefficient(1));
    }
    ...
    public void testDivideAndRoundQLastInplaceIterator() {
        RnsTool rnsTool;
        {
            ...
            RnsIterator inRns = new RnsIterator(in, n, k);

            rnsTool.divideAndRoundQLastInplace(inRns);
            Assert.assertEquals(0, inRns.coeff[0]);
            Assert.assertEquals(0, inRns.coeff[1]);
            ...
            inRns = new RnsIterator(in, n, k);
            rnsTool.divideAndRoundQLastInplace(inRns);
            Assert.assertEquals(4, inRns.coeff[0]);
            Assert.assertEquals(3, inRns.coeff[1]);
            ...
            inRns = new RnsIterator(in, n, k);
            rnsTool.divideAndRoundQLastInplace(inRns);
            Assert.assertEquals(3, inRns.coeff[0]);
            Assert.assertEquals(2, inRns.coeff[1]);
        }

        {
            ...
            Assert.assertEquals(0, inRns.coeff[0]);
            Assert.assertEquals(0, inRns.coeff[1]);
            Assert.assertEquals(0, inRns.coeff[2]);
            Assert.assertEquals(0, inRns.coeff[3]);
            Assert.assertEquals(0, inRns.coeff[4]);
            Assert.assertEquals(0, inRns.coeff[5]);
            ...
            inRns = new RnsIterator(in, n, k);
            rnsTool.divideAndRoundQLastInplace(inRns);
            Assert.assertTrue((3 + 2 - inRns.coeff[0]) % 3 <= 1);
            Assert.assertTrue((3 + 0 - inRns.coeff[1]) % 3 <= 1);
            Assert.assertTrue((5 + 0 - inRns.coeff[2]) % 5 <= 1);
            Assert.assertTrue((5 + 1 - inRns.coeff[3]) % 5 <= 1);
            Assert.assertTrue((7 + 5 - inRns.coeff[4]) % 7 <= 1);
            Assert.assertTrue((7 + 6 - inRns.coeff[5]) % 7 <= 1);
        }
    }
    ...
    public void testDivideAndRoundQLastNttInplaceIterator() {
        ...
        in[3] = 2;
        inRns = new RnsIterator(in, n, k);
        NttTool.nttNegacyclicHarvey(inRns.coeffIterators[0], nttTables[0]);
        NttTool.nttNegacyclicHarvey(inRns.coeffIterators[1], nttTables[1]);
        // We expect to get a zero output also in this case
        rnsTool.divideAndRoundQLastNttInplace(inRns, nttTables);
        NttTool.inverseNttNegacyclicHarvey(inRns.coeffIterators[0], nttTables[0]);
        Assert.assertEquals(0, inRns.coeff[0]);
        Assert.assertEquals(0, inRns.coeff[1]);
        ...
        NttTool.inverseNttNegacyclicHarvey(inRns.coeffIterators[0], nttTables[0]);
        // in[0] = 0, round(4/13) = 0
        Assert.assertTrue((53 + 1 - inRns.coeff[0]) % 53 <= 1);
        // in[1] = 1, round(12/13) = 1
        Assert.assertTrue((53 + 2 - inRns.coeff[1]) % 53 <= 1);
        ...
        NttTool.inverseNttNegacyclicHarvey(inRns.coeffIterators[0], nttTables[0]);
        // round(25/13) = 2
        Assert.assertTrue((53 + 2 - inRns.coeff[0]) % 53 <= 1);
        // round(35/13) = 3
        Assert.assertTrue((53 + 3 - inRns.coeff[1]) % 53 <= 1);
    }
