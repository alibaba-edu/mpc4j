 ...
 * <a href="https://github.com/microsoft/SEAL/blob/main/native/tests/seal/util/galois.cpp">
 * galois.cpp
 * </a>
 *
...
public class GaloisToolTest {
    ...
    @Test
    public void testApplyGaloisIterator() {
        ...
        RnsIterator inRns = new RnsIterator(in, 0, 8, 1);
        RnsIterator outRns = new RnsIterator(out, 0, 8, 1);

        galoisTool.applyGaloisRnsIter(inRns, inRns.k, 3, parms.coeffModulus(), outRns);
        for (int i = 0; i < 8; i++) {
            Assert.assertEquals(outTrue[i], outRns.coeff[i]);
        }
    }

    @Test
    public void testApplyGaloisNTTIterator() {
        EncryptionParameters parms = new EncryptionParameters(SchemeType.BFV);
        parms.setPolyModulusDegree(8);
        parms.setCoeffModulus(new long[]{17});
        parms.setPlainModulus(3);
        ...
        galoisTool.applyGaloisNttRnsIter(inRns, inRns.k, 3, outRns);
        for (int i = 0; i < 8; i++) {
            Assert.assertEquals(outTrue[i], outRns.coeff[i]);
        }
    }
