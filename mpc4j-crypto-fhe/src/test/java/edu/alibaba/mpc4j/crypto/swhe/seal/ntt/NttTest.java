...
public class NttTest {
    ...
    @Test
    public void testNegacyclicNttIterator() {
        int coeffCountPower = 1;
        Modulus modulus = new Modulus(0xffffffffffc0001L);
        NttTables tables = new NttTables(coeffCountPower, modulus);
        long[] poly = new long[]{0L, 0L};
        CoeffIterator coeffIterator = new CoeffIterator(poly, 0, 2);

        NttTool.nttNegacyclicHarvey(coeffIterator, tables);
        Assert.assertArrayEquals(new long[]{0L, 0L}, coeffIterator.getCoeff());

        poly = new long[]{1L, 0L};
        coeffIterator = new CoeffIterator(poly, 0, 2);
        NttTool.nttNegacyclicHarvey(coeffIterator, tables);
        Assert.assertArrayEquals(new long[]{1, 1}, coeffIterator.getCoeff());

        poly = new long[]{1L, 1L};
        coeffIterator = new CoeffIterator(poly, 0, 2);
        NttTool.nttNegacyclicHarvey(coeffIterator, tables);
        Assert.assertArrayEquals(new long[]{288794978602139553L, 864126526004445282L}, coeffIterator.getCoeff());
    }

    @Test
    public void testInverseNegacyclicNttIterator() {
        ...
        NttTool.inverseNttNegacyclicHarvey(coeffIterator, tables);
        for (int i = 0; i < 800; i++) {
            Assert.assertEquals(0, coeffIterator.getCoefficient(i));
        }
        ...
        NttTool.nttNegacyclicHarvey(coeffIterator, tables);
        NttTool.inverseNttNegacyclicHarvey(coeffIterator, tables);
        Assert.assertArrayEquals(temp, coeffIterator.getCoeff());
    }
