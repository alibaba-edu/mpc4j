...
public class EccLinearSolver {
     ...
     */
    private int rowEchelonForm(BigInteger[][] lhs, ECPoint[] rhs, boolean breakOnUnderDetermined) {
     ...
     */
    public SystemInfo solve(BigInteger[][] lhs, ECPoint[] rhs, ECPoint[] result, boolean solveIfUnderDetermined) {
        ...
        if (rhs.length == 1) {
            ...
            if (lhs[0].length == 1) {
                result[0] = rhs[0].multiply(lhs[0][0].modInverse(n)).normalize();
                return Consistent;
            }
            if (solveIfUnderDetermined) {
                ...
                for (int i = 0; i < result.length; ++i) {
                    if (!lhs[0][i].equals(BigInteger.ZERO)) {
                        result[i] = rhs[0].multiply(lhs[0][i].modInverse(n)).normalize();
                        return Consistent;
                    }
                }
                return Inconsistent;
        ...
        for (int i = 0; i < nzColumns.size(); ++i) {
            result[nzColumns.get(i)] = rhs[nzRows.get(i)].normalize();
        }
