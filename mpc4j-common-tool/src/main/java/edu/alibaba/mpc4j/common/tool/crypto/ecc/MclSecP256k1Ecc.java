 ...
 * @date 2021/12/13
 */
public class MclSecP256k1Ecc implements Ecc, AutoCloseable {
     ...
     * 本地MCL椭圆曲线服务单例
     */
    private final MclNativeSecP256k1Ecc mclNativeSecP256k1Ecc;
    /**
    ...
    public MclSecP256k1Ecc() {
        // 得到Bouncy Castle专门实现的优化SecP256k1曲线，转换成标准的椭圆曲线群参数
        ...
        // 得到本地MCL椭圆曲线服务的单例
        mclNativeSecP256k1Ecc = MclNativeSecP256k1Ecc.getInstance();
    ...
    public void precompute(ECPoint ecPoint) {
        ...
        if (!windowHandlerMap.containsKey(ecPoint)) {
            ByteBuffer windowHandler = mclNativeSecP256k1Ecc.precompute(ecPointToMclString(ecPoint));
    ...
    public void destroyPrecompute(ECPoint point) {
        if (windowHandlerMap.containsKey(point)) {
            ...
            mclNativeSecP256k1Ecc.destroyPrecompute(windowHandler);
    ...
    public ECPoint multiply(ECPoint ecPoint, BigInteger r) {
        ...
        if (windowHandlerMap.containsKey(ecPoint)) {
            ...
            String mulPointString = mclNativeSecP256k1Ecc.singleFixedPointMultiply(windowHandler, rString);
            return mclStringToEcPoint(mulPointString);
        } else {
            String pointString = ecPointToMclString(ecPoint);
            String mulPointString = mclNativeSecP256k1Ecc.singleMultiply(pointString, rString);
            return mclStringToEcPoint(mulPointString);
        }
    }
    ...
    public ECPoint[] multiply(ECPoint ecPoint, BigInteger[] rs) {
        ...
        if (windowHandlerMap.containsKey(ecPoint)) {
            // 先判断给定点是否已经进行了预计算，如果进行过预计算，则用预计算乘法处理
            ByteBuffer windowHandler = windowHandlerMap.get(ecPoint);
            String[] mulPointStrings = mclNativeSecP256k1Ecc.fixedPointMultiply(windowHandler, rStrings);
            return Arrays.stream(mulPointStrings).map(this::mclStringToEcPoint).toArray(ECPoint[]::new);
        } else {
            String pointString = ecPointToMclString(ecPoint);
            String[] mulPointStrings = mclNativeSecP256k1Ecc.multiply(pointString, rStrings);
            return Arrays.stream(mulPointStrings).map(this::mclStringToEcPoint).toArray(ECPoint[]::new);
        }
    }
    ...
    public EccType getEccType() {
        return EccType.MCL_SEC_P256_K1;
    }
