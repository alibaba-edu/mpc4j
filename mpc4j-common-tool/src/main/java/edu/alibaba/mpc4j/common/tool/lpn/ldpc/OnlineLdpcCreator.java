...
class OnlineLdpcCreator extends AbstractLdpcCreator {
     ...
     */
    OnlineLdpcCreator(LdpcCreatorUtils.CodeType codeType, int ceilLogN) throws IllegalStateException {
        if (ceilLogN < LdpcCreatorUtils.MIN_LOG_N || ceilLogN > LdpcCreatorUtils.MAX_LOG_N) {
            throw new IllegalArgumentException("OnlineLdpcCreator ONLY supports ceilLogN in: " +
                "[" + LdpcCreatorUtils.MIN_LOG_N + ", " + LdpcCreatorUtils.MAX_LOG_N + "]");
        }
        initParams(codeType, ceilLogN);
        // 读取生成对应的参数和种子。
        initFromFile();
